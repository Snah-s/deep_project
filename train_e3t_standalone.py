...
# --- Shaping DENSO (CRITICO) ---
# El paquete overcooked_ai tiene los distance-rewards COMENTADOS: solo da reward por evento
# (placement/dish/soup), que una politica aleatoria casi nunca dispara -> sin gradiente -> colapso.
# Aqui lo calculamos: potencial = -dist(agente, subobjetivo del greedy). PBRS (delta de potencial),
# no explotable, se annela con el shaping. Es lo que permite bootstrapear la tarea desde cero.
DENSE_SHAPING = True
DENSE_COEF    = 1.0    # reward por cada celda mas cerca del subobjetivo actual
...
class OvercookedGymEnv(gym.Env):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # shaping denso: un greedy solo para leer el subobjetivo actual del asiento controlado
        self._shaper = GreedyFullTaskPolicy(seed=0)
        self._shaper.set_mdp(self.mdp); self._shaper.set_agent_index(self.seat)
        self._prev_dist = None; self._prev_tgt = None
        obs = encode_padded(self.mdp, self.oc.state, self.seat)
        return obs, {"layout": self.layout, "seat": self.seat}

    def _dense_shaping(self, next_state):
        """PBRS: reward por acercarse al subobjetivo del greedy (delta de -distancia)."""
        if not DENSE_SHAPING:
            return 0.0
        try:
            tgt = self._shaper._choose_target(next_state)
            if tgt is None:
                self._prev_dist = None; self._prev_tgt = None; return 0.0
            pos = next_state.players[self.seat].position
            d = abs(pos[0] - tgt[0]) + abs(pos[1] - tgt[1])
            delta = (self._prev_dist - d) if (self._prev_tgt == tgt and self._prev_dist is not None) else 0.0
            self._prev_dist = d; self._prev_tgt = tgt
            return float(delta)
        except Exception:
            self._prev_dist = None; self._prev_tgt = None; return 0.0
    ...
    def step(self, action):
        ...
        dense = self._dense_shaping(next_state)                       # shaping denso (CRITICO)
        reward = float(sparse_team) + self.shaping_coef * (shaped_own + DENSE_COEF * dense + aux)
